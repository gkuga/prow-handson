...
例えばK8sでは、ReplicaSetというリソースが作成されると、指定されたPodのレプリカ数を維持するようにPodリソースを調整するコントローラーというデーモンが動く。ProwではProwJobが作成されると、Plankというサービスが、ProwJobに記述されたジョブをPodリソースとして作成して実行する。そして、成功ならProwJobのステータスをSuccessにして失敗ならFailureにするなどしている。ProwJobの作成を行っているのは、定期的なジョブならばHorologiumサービスが作成し、PRが更新されるときはHookサービスのTriggerプラグインが作成する。
